...
П. 7.6 (перенос старых серий) сделан 2026-09-05 миграцией `036`, накачена.
Спека закрыта целиком; осталась уборка мёртвого кода `series_id` — см. конец 7.6.
...
Дальше — миграция, удаляющая `lessons.series_id`, `UpdateSeriesRequest`,
`DELETE /lessons/series/:seriesId`, `PATCH /lessons/series/:seriesId`,
`POST /lessons/bulk` и `SeriesDialog` на фронте. Колонка уже пуста, механизм
мёртв — осталось убрать код. Клиентский `generateDates()` удалён раньше, вместе
с хвостами фазы 3.
