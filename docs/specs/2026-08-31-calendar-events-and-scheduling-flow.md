...
### 9.3. Пробные комнаты и редеплой — закрыто

`handlers/call.go` держал активные пробные комнаты в памяти процесса —
`quickRooms map[string]*quickRoom` с вытеснением через 3 часа. При редеплое на
Railway посреди пробного урока `GET /public/quick/:id/guest-token` начинал
отдавать `404 room not found or ended`: преподаватель в комнате оставался (его
токен уже выдан), а потенциальный ученик войти не мог. Фаза 5 вынесла кнопку
«Начать пробный урок» на событие календаря, пробных стало заметно больше — и
вероятность попасть редеплоем в живой урок вместе с ними.

> **Как закрыто.** Источником истины стал сам LiveKit. `StartQuickRoom` заводит
> комнату явно (`CreateRoom` с `Metadata: tutorID`), три чтения —
> `ListRooms(Names: …)` через хелпер `quickRoomOwner`, владелец берётся из
> метаданных комнаты. Мапа, мьютекс и `time.AfterFunc` удалены; TTL заменили
> штатные `EmptyTimeout` / `DepartureTimeout` (по часу).
>
> **Почему не таблица и не Redis.** Состояние — одно поле `tutorID`, и оно
> нужно единственной проверке прав; «жива ли комната» LiveKit знает и так, а
> клиент к нему в хендлере уже был (через него шёл `DeleteRoom`). Отдельная
> таблица дублировала бы чужое состояние и добавила бы слой на пустом месте.
>
> **Побочно исправлено:** недоступность LiveKit теперь отвечает `503`, а не
> `404` — фронт (`join/room/[id]/page.tsx`) приравнивает `ended` к концу урока
> и уводит гостя на финальный экран без возврата, так что сетевой сбой не
> должен выглядеть как завершённый урок. `GET /public/quick/:id/status` получил
> rate-limit (был единственной публичной quick-ручкой без него, а теперь каждый
> вызов идёт в LiveKit).
>
> Регрессия закреплена тестом `TestQuickRoom_SurvivesProcessRestart`: комната
> поднимается на одном хендлере, читается вторым — то есть буквально «новый
> процесс поверх того же LiveKit».
