...
фазы 4 и 5 готовы и ждут мержа — миграцию `035` перед деплоем накатить.
Не сделан п. 7.6 — перенос старых серий, причина там же.
...
> **Отложено, не сделано вместе с остальной фазой 3.** На проде 31 серия и 2834
> урока, из них 1281 будущий у боевого аккаунта; типичная серия — ровно 200
> уроков до 2028–2030 годов (артефакт бага, который чинила фаза 0), а фактические
> дни недели у таких серий выглядят как `{1,2,3,4,5,6,7}`. Правило,
> восстановленное по таким строкам, окажется ежедневным и бессрочным, и первое же
> «изменить все следующие» размножит уроки.
>
> Порядок должен быть обратный: сначала почистить хвосты старых серий (или
> закрыть их `ends_on` по факту), и только потом переносить на `rule_id`. До тех
> пор старые серии живут на `series_id` со своим `SeriesDialog`, новые — на
...
| 5 | Кнопка «Начать пробный» на событии, ICS | 035 | 1–2 дня | ✅ миграция 035 **не накачена** на прод |
