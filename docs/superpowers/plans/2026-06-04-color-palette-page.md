# Color Palette Dev Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a dev page at `/dev/colors` that renders all global CSS color tokens with swatches and a light/dark toggle.

**Architecture:** Single client component outside all route groups — no layout wrapper, no auth. All token values resolved at runtime via `getComputedStyle` so they reflect the live theme. Theme toggle sets `.dark` on `<html>` and persists to `localStorage`.

**Tech Stack:** Next.js App Router, Tailwind CSS, CSS custom properties (`globals.css`)

---

### Task 1: Create `/dev/colors` page

**Files:**
- Create: `frontend/src/app/dev/colors/page.tsx`

- [ ] **Step 1: Create the file with section data and token resolver**
...
- [ ] **Step 2: Запустить dev-сервер и проверить страницу**

```bash
cd frontend && npm run dev
```

Открыть `http://localhost:3000/dev/colors`. Ожидаемое: все секции видны, свотчи отображают цвета, переключатель темы работает, hex-значения заполняются.

- [ ] **Step 3: Commit**

```bash
git add frontend/src/app/dev/colors/page.tsx
git commit -m "feat: add /dev/colors token palette page"
```
