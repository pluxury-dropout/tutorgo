...
## Carry-forward перед включением enforcement (из финального ревью)

Ничего из этого не блокирует merge backend-фазы 1, но обязано быть закрыто
до того, как enforcement реально включат для платящих:

1. **Деплой миграции 018 + кода — атомарно (runbook).** Если новый код поедет
   раньше миграции — каждый `POST /auth/register` даст 500 (INSERT в
   несуществующую таблицу). Если миграция поедет раньше кода и в этот зазор
   старый код обслужит регистрацию — создастся репетитор **без строки подписки**,
   которого бэкфилл уже не покроет → он навсегда `blocked`. Вывод: миграция и код
   выкатываются одним релизом.
2. **`POST /subscription/confirm` = «выдать себе платный период бесплатно».**
   #1 pre-launch gate: до реальных платежей заменить на webhook провайдера с
   проверкой подписи (или удалить). Пока провайдера нет — безвредно.
3. **WS-роут доски (`handlers/whiteboard_ws.go` `ServeWS`) вне гейта.**
   Заблокированный репетитор с валидным JWT сохраняет живое редактирование
   существующих досок (создавать/грузить не может — те HTTP-роуты гейтятся).
   Роут нельзя просто перенести под `auth` (он же обслуживает гостей по invite).
   Фикс: проверка подписки внутри `authorizeWS` только для JWT-подключений (тьютор).
