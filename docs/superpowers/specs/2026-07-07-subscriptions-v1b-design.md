...
**Уточнение по ревью (реализовано, 2026-07-08):** conditional UPDATE платежа и
UPDATE подписки выполняются **в одной транзакции** (репо-методы
`MarkSuccessAndStartPeriod` для webhook, `MarkSuccessAndRenew` для renew) — иначе
сбой между двумя шагами давал «оплачено, но не активировано» (webhook,
передоставка становилась no-op) и двойное списание (renew, ретрай завтра с новым
по-дневным ключом). Плюс **trilean-семантика статусов** везде, где решение
необратимо: подтверждённый `success` → активировать; подтверждённый `failed` →
закрыть строку; **всё остальное (ошибка `CheckStatus`, не-терминальный статус) →
ничего не делать и переспросить следующим тиком** — строка остаётся `pending`,
новое списание за период заблокировано. Recovery-проход в `RenewDue`
(`ListPeriodPayments` по префиксу `tutor:period_end:`) реконсилирует pending-строки
прошлых дней ДО любого нового Charge; сегодняшнюю строку не трогает (может быть
в полёте у другого инстанса).
