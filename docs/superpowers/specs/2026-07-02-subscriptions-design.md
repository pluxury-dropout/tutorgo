...
- **Крон/фоновая задача** — не нужны: состояние вычисляется на чтении.
  Понадобятся только для проактивных email-напоминаний или автосписания с карты.
- **Проактивные email-напоминания** («триал заканчивается через 3 дня») —
  **вторая итерация**. Требуют отдельной подсистемы: почтовый провайдер (сейчас
  в `config` нет SMTP/email), шаблоны писем и планировщик. В первой итерации
  предупреждение показывает **только внутри-продуктовый баннер**: фронт по
  `state == "grace"` (и опционально по близости `period_end` в `active`) рисует
  «тариф истекает/истёк, оплатите». Гейт доступа это уже полностью закрывает.
