...
export function useExcalidrawSync(
  ...
  const connect = useCallback(async () => {
    ...
    // Явный токен доски (invite гостя или ученика) перебивает сессионный: он
    // выдан ровно на эту доску. Раньше он уходил в getTokenAsync ФОЛБЭКОМ, а тот
    // отдаёт localStorage.tg_token, если он есть, — и гость, у которого в том же
    // браузере жила чужая tutor-сессия, стучался в WS чужим JWT. Сервер честно
    // отвечал 401 (страница принадлежит другому репетитору), клиент уходил в
    // вечное «переподключение…», а вместе с ним пропадали и коллабораторы с
    // follow. Своя доска репетитора токен не передаёт — там как было: ждём
    // возможный проактивный рефреш (HTTP-перехватчик axios обновляет токен
    // асинхронно, WS этот путь минует) и берём сессионный.
    const wsToken = token ?? (await getTokenAsync())
