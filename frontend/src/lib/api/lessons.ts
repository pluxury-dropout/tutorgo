...
export const lessonsApi = {
  ...
  delete: (id: string) => api.delete(`/lessons/${id}`).then(() => id),
  getAttendance: (lessonId: string) =>
    api.get<AttendanceRecord[]>(`/lessons/${lessonId}/attendance`).then((r) => r.data),
  updateAttendance: (lessonId: string, data: AttendanceRecord[]) =>
    api.put<AttendanceRecord[]>(`/lessons/${lessonId}/attendance`, data).then((r) => r.data),
}
